...
class SynchronousTestingSpec extends AnyWordSpecLike with BeforeAndAfterAll {
  ...
  "A counter" should {
    "synchronously increase its counter" in {
      ...
      counter ! Inc // counter has already received the message -- All this happen in the calling thread
      assert(counter.underlyingActor.count == 1)
    }

    "synchronously increase its counter at the call of the receive function" in {
      val counter = TestActorRef[Counter](Props[Counter])
      counter.receive( Inc ) // counter has already received the message -- All this happen in the calling thread
      assert(counter.underlyingActor.count == 1)
    }
